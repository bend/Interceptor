...
void BackendConnector::doPost(std::pair<Packet, std::function<void(Http::Code)>> data)
{
...
void BackendConnector::doForward(Packet& packet, std::function<void(Http::Code)> callback)
{
  LOG_DEBUG("BackendConnector::doForward()");
  m_state &= ~CanWrite;
  m_connection->asyncWrite(std::get<const char*>(packet), std::get<size_t>(packet),
	  m_strand.wrap(
		std::bind(&BackendConnector::handlePacketForwarded, shared_from_this(), std::placeholders::_1, callback)));
}

void BackendConnector::handlePacketForwarded(const boost::system::error_code& error, 
	std::function<void(Http::Code)> callback) {
