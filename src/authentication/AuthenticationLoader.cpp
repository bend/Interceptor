...
namespace Interceptor::Authentication {
  ...
  bool AuthenticationLoader::loadAuthentications(std::vector<AuthenticationCPtr>& auths)
  {		
	for(auto & a : auths)
	{
  ...
  AbstractAuthenticationPtr AuthenticationLoader::get(const std::string& name) const
  {
